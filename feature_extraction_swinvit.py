...
# SwinUNETR에서 encoder만 따로 생성하려면, SwinUNETR과 동일한 파라미터로 생성
model = SwinUNETR(
    in_channels=1,
    out_channels=14,
    feature_size=48,
...
# encoder만 따로 추출
encoder = model.swinViT  # <--- 이 부분이 SwinTransformer3D 객체

# encoder만 따로 저장
torch.save(encoder.state_dict(), "weights/swinvit_encoder.pth")
print("SwinViT encoder만 따로 저장 완료!")
